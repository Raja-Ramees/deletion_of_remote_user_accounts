...
def create_user_on_server(server, ssh_user, ssh_password, users):
    try:
        ...
        for user in users:
            ...
            shell.send(f"sudo useradd -m  {user}\n")
            ...
            if "password" not in output:
                print(f"User {user} successfully created from {server}.")
            else:
                print(f"Failed to create user {user} from {server}. Error: {output}")
...
users_to_create = ["usr1", "usr2", "usr3", "usr4", "usr5"]
